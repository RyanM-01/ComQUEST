        ...
        <form action="{{ route('admin.opsi.store', $question->id) }}" method="post">
          ...
          <div class="form-check" tabindex="0">
            <span>&nbsp</span>
              ...
              <form action="{{  route('admin.opsi.update', [$bab->id, $quiz->id, $question->id, $option]) }}" method="POST" style="display:inline-block;">
                @csrf
                @method('PUT')
              ...
              <form action="{{ route('admin.opsi.destroy', [$bab->id, $quiz->id, $question->id, $option] ) }}" method="POST" style="display:inline-block;">
                @csrf
                @method('DELETE')
                <button class="button-container btn tambahopsi" type="submit">
                  <i class="trash uil uil-trash-alt"></i> Hapus opsi
                </button>
              </form>
